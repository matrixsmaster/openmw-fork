...
InputWrapper::InputWrapper(SDL_Window* window, osg::ref_ptr<osgViewer::Viewer> viewer, bool grab) :
    ...
    MyGUI::KeyCode::Enum InputWrapper::SDL2MGKeyCode(SDL_Keycode code)
    {
        if (!mgmap_tab.count(code)) return MyGUI::KeyCode::None;

        return mgmap_tab[code];
    }
